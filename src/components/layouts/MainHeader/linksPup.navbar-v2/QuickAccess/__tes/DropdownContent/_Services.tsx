...
/**
 * @function
 * @description This component renders services section in nav-bar v2 quick access dropdown.
 * @returns {ReactElement} A div containing services section.
 */
export const Services = () => {
    return (
      <div className="text-dis rounded-t-lg w-1/2 bg-gray-100 p-4">
  ...
  const Service =({name , link , icon}:{name:string , link:string , icon:string})=>{
    return(
    <div className="flex items-center gap-2 text-dis cursor-pointer">
      <div className="w-7 h-7 rounded-[3px] flex items-center justify-center shadow-inner">
        <i  className={`fa-solid ${icon} `} />
      </div>
      <Link href={link} className=" hover:text-bl">
        {name}
      </Link>
    </div>
    )

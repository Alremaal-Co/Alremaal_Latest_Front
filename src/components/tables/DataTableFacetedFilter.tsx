...
import { FacetedFilterOption } from "./types.utils.tables";
...
export function DataTableFacetedFilter<TData, TValue>({ column, title, options }: { column?: Column<TData, TValue>, title?: string, options: FacetedFilterOption[] }) {
                ...
                <div className="hidden space-x-1 lg:flex">
                  {selectedValues.size > 2 ? (
                    ...
                    options
                      ...
                      .map((option) => (<Badge variant="secondary" key={option.value} className="rounded-sm px-1 font-normal">{option.label}</Badge>))
                  )}
                </div>
              </>
            )}
